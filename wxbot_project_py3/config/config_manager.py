...
class ConfigManager(object):
    ...
    def get_wechat_config(self):
        uin = self.cp['wechat']['uin']
        last_login = self.cp['wechat']['last_login']
        conf = [
            self.cp['wechat']['uuid'],
            self.cp['wechat']['redirect_uri'],
            int(uin if uin else 0),
            self.cp['wechat']['sid'],
            self.cp['wechat']['skey'],
            self.cp['wechat']['pass_ticket'],
            self.cp['wechat']['synckey'],
            self.cp['wechat']['device_id'],
    ...
    def get_wechat_media_dir(self):
        prefix = self.get('setting', 'prefix')
        path = prefix + self.cp['setting']['mediapath']
        return {
