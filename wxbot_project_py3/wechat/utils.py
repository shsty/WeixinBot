...
def post(url, params, jsonfmt=True):
    ...
    Log.debug(params)
    if jsonfmt:
        request = urllib.request.Request(url=url, data=json.dumps(params, ensure_ascii=False).encode('utf8'))
        request.add_header(*Constant.HTTP_HEADER_CONTENTTYPE)
    else:
        request = urllib.request.Request(url=url, data=urllib.parse.urlencode(params).encode('utf8'))
